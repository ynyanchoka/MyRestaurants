...
//
//
////    public static RestaurantDetailFragment newInstance(Business restaurant) {//used instead of a constructor and returns a new instance of our RestaurantDetailFragment.
...
//
//    }
////only inflates the photo menu if the user has navigated to RestaurantDetailFragment from the "Saved Restaurants" list. If they did not, only the main menu is inflated.
//    @Override
...
//        return false;
//    }
////This is an implicit intent that will instruct Android to automatically access the device's camera. MediaStore (Links to an external site.) is a built-in Android class that handles all things media, and ACTION_IMAGE_CAPTURE (Links to an external site.) is the standard intent that accesses the device's camera application.
    ...
    public void onClick(View v) {
        ...
        if (v == mPhoneLabel) {
            ...
            Intent mapIntent = new Intent(Intent.ACTION_VIEW,
                    Uri.parse("geo:" + mRestaurant.getCoordinates()
                                    + "," + mRestaurant.getCoordinates().getLongitude()
                    + "?q=(" + mRestaurant.getName() + ")"));
            startActivity(mapIntent);
        } if (v == mSaveRestaurantButton) {
